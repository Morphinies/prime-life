...
export class TasksRepository {
  ...
  async findAll(filters: TaskListFilters): Promise<Task[]> {
    ...
    const whereByPeriod: Record<'day' | 'week' | 'month' | 'year', string> = {
    ...
    if (search) {
      values.push(`%${search}%`);
      conditions.push(
        `(title ILIKE $${values.length} OR COALESCE(description, '') ILIKE $${values.length} OR COALESCE(section, '') ILIKE $${values.length} OR COALESCE(project, '') ILIKE $${values.length})`
      );
    }
